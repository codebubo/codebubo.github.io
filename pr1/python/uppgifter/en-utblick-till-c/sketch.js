...
function setup() {
  let canvas = createCanvas(720, 550);
  ...
  toggleButton.parent("sketch-holder");
...
function draw() {
  for (let i = 0; i < N; i++) {
    ...
    y[i] = Math.sin(phi * i - omega * tt);
    x[i] = (w / (N - 1)) * i;
  }
  render();
  if (anim) {
    tt += deltaTime / 1000; // milli seconds to seconds
  }
}
