...
**Lösningsförslag för antalsavgörandet** (andra deluppgiften)
...
elif årtal % 4 == 0 and årtal % 100 == 0:
...
if not årtal % 400:
    print(f"År {årtal} är ett skottår")
elif not årtal % 4 and not årtal % 100:
    print(f"År {årtal} är inte ett skottår")
elif not årtal % 4:
    print(f"År {årtal} är ett skottår")
else:
...
**Variant 3**, uppbyggnad av programmet med nästlad
`if`-sats istället för `and`-operatorn.
...
# används. Här används egenskapen att om tal a är jämnt
# delbart med tal b så gäller att a % b är noll.
# Istället för sammansättande booleska operatorer så
# använder programmet en nästlad if-sats.
