...
[övning 1](#%C3%B6vning-1) så ska denna funktion **returnera** värdet på
...
är givet i [Övning 9 i avsnittet om loopar](../loopar/index.qmd#%C3%B6vning-9)
...
Även om programmet i [Övning 3](##%C3%B6vning-3) gör sitt jobb, så är
...
Du har tidigare [skrivit ett program](../loopar/index.qmd#%C3%B6vning-4) där du
...
Differenskvoten av en matematisk funktion tecknas enligt
$\frac{f(x+h)-f(x)}{h}$, där $h$ ofta är ett tal i närheten av noll.

Skriv en funktion `diff(x, h)` som returnerar differenskvoten för den
matematiska funktionen $y=2x^3 - 5x^2+8x-1$ för givna värden på `x` och `h`.

Använd sedan funktionen i ett program som gör en tabell över värden på
differenskvoten för ett givet värde på `x` och olika värden på `h`.
Utmatningen av en körning kan se ut enligt nedan:
...
#| echo: false
def diff(x, h):
    term_1 = 2*(x+h)**3 - 5*(x+h)**2 + 8*(x+h) - 1
    term_2 = 2*x**3 - 5*x**2 + 8*x - 1
    ret_val = (term_1 - term_2) / h
    return ret_val

print("x".rjust(3), end="")
print("h".rjust(13), end="")
print("Differenskvot".rjust(23), end="\n")
print(40*"-")
h = 0.1 # Initial value of h
k = 0 # Counter for adjustment in table
x = 2 # Current value of x
while h > 0.00001:
    diffkvot = diff(x, h)
    print(f"{x:.1f}".rjust(4), end="")
    print(f"{h}".rjust(13 + k), end="")
    print(f"{diffkvot:.3f}".rjust(18 - k))
    h /= 10
    k += 1
```
